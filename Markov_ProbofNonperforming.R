...
Migration <- read_excel("DataMigration.xlsx") %>%
  mutate(
    start_date = ymd(`Nuevo en`),     # convierte AAAA-MM-DD a Date
    start_state = as.integer(NPL)     # 0..25
  )
...
# ---- READ PROBABILITY MATRIX (Sheet1) ----
# In your screenshots the numeric block is C3:AE27 and row labels (state 0..25) are in B3:B27,
...
# ---- OPTIONAL: read the cumulative matrix (Sheet2) ----
# This lets us sample a next state with findInterval() without re-cumsumming each time.
...
# --- Simulación (usa Pc o P como antes) ---
...
# --- CLEAN TRANSITION MATRICES (replace NA by 0, renormalize, cumulate) ---

# Start from the raw probability matrix P you already loaded
P_clean <- P
P_clean[is.na(P_clean)] <- 0                     # impossible = 0%, not NA

# optional: make a state (e.g., 25) absorbing
# P_clean["25", ] <- 0; P_clean["25", "25"] <- 1

# Renormalize rows to sum to 1 (in case your sheet has rounding zeros/edits)
rs <- rowSums(P_clean)
...
# 1) Run the Markov simulation using the cumulative matrix Pc
...
# 2) Tidy result to long form (Id_client, date, state)
library(tibble); library(tidyr); library(dplyr); library(lubridate)
...
# 3) Join any client attributes you need (loan amount, type, etc.)
sim_long <- sim_long %>%
  mutate(Id_client = as.numeric(Id_client))


## --- 3) Join attributes and build a working table (sim_full) ---
...
## --- 4) Simple EAD policy example ---
# - Simple: 100% draw only in the start month
# - Revolvent: constant exposure = loan amount (adjust to your rules anytime)
...
## --- 5) Portfolio metrics (example: NPL = states >= 3) ---
...
## (optional) state distribution by month, exposure-weighted
state_expo <- sim_full %>%
  group_by(date, state) %>%
...
# 4) (Optional) simple EAD policy just to have a number:
...
# 5) Quick look at portfolio state mix through time
...
## ---- (Optional) Portfolio summary example ----
